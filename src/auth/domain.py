...
from src.core.settings import ACCESS_TOKEN_EXPIRES
from src.core.settings import REFRESH_TOKEN_EXPIRES
...
class UserAuthenticateUseCase:
    def __init__(self, context: DbConnectionHandler, payload: OAuth2PasswordRequestForm) -> None:
    ...
    async def execute(self):
        user = await self._user_repository.get(query.eq(
            UserModel.email, self._payload.username
        ))
    ...
    def _create_tokens(self, user: User):
        access_token = create_token({"sub": user.email}, timedelta(minutes=self._access_expires))
        refresh_token = create_token({"sub": user.nickname}, timedelta(days=self._refresh_expires))
